...
DECLARE @BATCH INT=1 WHILE( @BATCH <= 6 ) BEGIN UPDATE TOP(2000000) $(schema).CRMM_BABR  SET  HOME_SYSTEM = @crm_target_ls WHERE  HOME_SYSTEM = @crm_source_ls SET @BATCH=@BATCH + 1 END -- (15 mins)
