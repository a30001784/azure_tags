...
DECLARE @BATCH INT=1 WHILE( @BATCH <= 5 ) BEGIN UPDATE TOP(2000000) $(schema).CRMM_BUT_FRG0040  SET   HOME_SYSTEM = @crm_target_ls WHERE  HOME_SYSTEM = @crm_source_ls SET @BATCH=@BATCH + 1 END -- (2 mins)