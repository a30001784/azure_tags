...
DECLARE @BATCH INT=1 WHILE( @BATCH <= 4 ) BEGIN UPDATE TOP(2000000) $(schema).COMM_PRODUCT   SET LOGSYS = @crm_target_ls WHERE  LOGSYS = @crm_source_ls SET @BATCH=@BATCH + 1 END -- (2 mins)