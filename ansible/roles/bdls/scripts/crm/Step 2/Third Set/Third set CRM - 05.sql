...
DECLARE @BATCH INT=1 WHILE( @BATCH <= 2 ) BEGIN UPDATE TOP(2000000) $(schema).EDIDC  SET  RCVPRN = @crm_target_ls WHERE  RCVPRN = @crm_source_ls SET @BATCH=@BATCH + 1 END -- (4 mins)
