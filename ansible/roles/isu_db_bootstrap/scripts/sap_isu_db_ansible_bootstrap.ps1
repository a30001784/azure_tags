<#
.SYNOPSIS
  Script to complete the post configuration tasks for ISU Database
 
.DESCRIPTION
  The script is used to complete the post configuration tasks for the ISU DB build.
...
.EXAMPLE
  .\sap_isu_db_ansible_bootstrap -SID "***" -storName "***" -storKey "***"
#>
...
if($DataDisks.Count -eq 0) {
  ...
  # New Data Disk
  New-VirtualDisk -StoragePoolFriendlyName "StoragePool" -FriendlyName "Data" -Interleave "65536" -NumberOfColumns "15" -ResiliencySettingName simple -Size 15360GB | `
  ...
  # New Backup Disk
	New-VirtualDisk -StoragePoolFriendlyName "StoragePool" -FriendlyName "Backup" -Interleave "65536" -NumberOfColumns "15" -ResiliencySettingName simple -UseMaximumSize | `
		Initialize-Disk -PartitionStyle GPT -PassThru | `
...
New-PSDrive -Name Z -PSProvider FileSystem -Root "\\aaasapautomationsa.file.core.windows.net\sapptia" -Credential $credential -Persist

# Copy Azure FileStore to Local Packages Dir 
Robocopy "Z:\installFiles\" "C:\Packages\SAP\" *.cab
Robocopy "Z:\installFiles\MS-Visual-C-2005-SP1-Redistributable" "C:\Packages\SAP\"
