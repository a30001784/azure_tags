...
# Start SAP System
& "C:\Program Files\SAP\hostctrl\exe\sapcontrol" -nr $instance -host $saphost -user $sapadmuser $sapadmpwd -function StartSystem

# Wait for all SAP Instances to Start
Do {
    & "C:\Program Files\SAP\hostctrl\exe\sapcontrol" -nr $instance -host $saphost -user $sapadmuser $sapadmpwd -function GetSystemInstanceList > $Path
    get-content $Path | select -Skip 4 | set-content "$Path-temp"
    move "$Path-temp" $Path -Force
    $check = import-csv $Path | where-object {$_.dispstatus -eq "GRAY"} | Format-Table | Out-String
    }
While ($check -like "*GRAY*")

Do {
    & "C:\Program Files\SAP\hostctrl\exe\sapcontrol" -nr $instance -host $saphost -user $sapadmuser $sapadmpwd -function GetSystemInstanceList > $Path
    get-content $Path | select -Skip 4 | set-content "$Path-temp"
    move "$Path-temp" $Path -Force
    $check = import-csv $Path | where-object {$_.dispstatus -eq "YELLOW"} | Format-Table | Out-String
    }
While ($check -like "*YELLOW*")
